...
{
    public class VolCalModel
    {
        public double[,] TopDepths { get; set; }
        public double[,] BaseDepths { get; set; }
        public double FluidContactDepth { get; set; }
        public double CalculatedVolume { get; private set; }

        public VolCalModel()
        {
            // Initialize TopHorizonDepths
            InitializeTopDepths();

            //Initialize BaseHorizonDepths = TopHorizonDepths + 100 meters
            CalculateBaseDepths();
        ...
        private void InitializeTopDepths()
        {
            // Initialize TopHorizonDepths with dummy values
            TopDepths = new double[,]
            {             
        ...
        private void CalculateBaseDepths()
        {
            // Initialize BaseHorizonDepths = TopHorizonDepths + 100 meters
            BaseDepths = new double[TopDepths.GetLength(0), TopDepths.GetLength(1)];

            for (int i = 0; i < TopDepths.GetLength(0); i++)
            {
                for (int j = 0; j < TopDepths.GetLength(1); j++)
                {
                    BaseDepths[i, j] = TopDepths[i, j] + (100 * 3.28084);
                }
            }
        }
        ...
        {
            // Ensure that TopHorizonDepths, BaseHorizonDepths
            if (TopDepths == null || BaseDepths == null)
            {
                throw new InvalidOperationException("TopDepths and BaseDepths must be initialized before calculating volume.");
            }

            // Ensure that TopHorizonDepths and BaseHorizonDepths have the same dimensions
            if (TopDepths.GetLength(0) != BaseDepths.GetLength(0) ||
                TopDepths.GetLength(1) != BaseDepths.GetLength(1))
            {
                throw new InvalidOperationException("TopDepths and BaseDepths must have the same dimensions.");
            }

            if (FluidContactDepth<=0 )
            {
                throw new InvalidOperationException("Select FluidContactDepth Value and select unit.");
            }

            double lateralGridCellArea = 200 * 200; // Area of each lateral grid cell in square feet
            ...
            for (int i = 0; i < TopDepths.GetLength(0); i++)
            {
                for (int j = 0; j < TopDepths.GetLength(1); j++)
                {
                    //// Calculate the volume contribution of the current cell
                    
                    double cellHeight = BaseDepths[i, j] - TopDepths[i, j];

                    // cell height doesn't go below zero
                    cellHeight = cellHeight < 0 ? 0 : cellHeight;

                    // cell height doesn't exceed the fluid contact depth
                    cellHeight = cellHeight > FluidContactDepth ? FluidContactDepth : cellHeight;

                    volume += cellHeight * lateralGridCellArea;
                }
            }

            // Assign the calculated volume to the property
